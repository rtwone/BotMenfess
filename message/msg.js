  ...
  try {
    ...
    const isOwner =
      ownerNumber == sender
        ? true
        : [
            "6285791458996@s.whatsapp.net",
            "6285175222272@s.whatsapp.net",
          ].includes(sender)
        ? true
        : false;
    ...
    if (!msg.key.fromMe && secreto.find((i) => i.sender === sender)) {
      ...
      if (dbx.status === "ENTER-MESSAGE") {
        if (["conversation", "extendedTextMessage"].includes(msg.type)) {
          if (chats.startsWith(prefix + "balas_secreto"))
            return reply(
              `Bales satu-satu kak, masukin pesan balasan yang sebelumnya!`
            );
          if (chats.startsWith(prefix))
            return reply(
              `Jangan diawali dengan simbol jika kamu ingin membalas pesan menfess`
            );
          ...
          var button_balas = {
            ...
            buttons: [
              {
                buttonId: `.connect_chat ${sender}_with_${dbx.pengirim}`,
                buttonText: { displayText: `Connect Chat` },
                type: 1,
              },
            ],
          ...
          conn
            .sendMessage(dbx.pengirim, { forward: msg }, { quoted: pes })
            .then((res) => conn.sendMessage(dbx.pengirim, button_balas2));
          reply(`Sukses mengirimkan balasan`);
        }
    ...
    switch (command) {
      ...
      case prefix + "menfess":
        ...
        if (args.length < 2)
          return reply(
            `Silahkan masukkan format seperti contoh dibawah\n\nContoh format:\n${command} Nomer|Nama|Pesan`
          );
        ...
        if (!number_to)
          return reply(
            `Format salah, masukkan nomer tujuan\n\nContoh Format:\n${command} Nomer|Secret|Hai Kak`
          );
        if (isNaN(number_to))
          return reply(
            `Tujuan Harus Berupa Nomor!\n\nContoh Format:\n${command} ` +
              botNumber.split("@")[0] +
              "|Secret|Hai Kak"
          );
        ...
        if (!sender_name)
          return reply(
            `Format salah, masukkan nama Anda atau nama rahasia\n\nContoh Format:\n${command} ` +
              number_to.split("@")[0] +
              "|Nama|Hai Kak"
          );
        if (!msg_send)
          return reply(
            `Format salah, masukkan pesan yang akan dikirim\n\nContoh Format:\n${command} ${
              number_to.split("@")[0]
            }|${sender_name}|Hai Kak`
          );
        var cek_number = await conn.onWhatsApp(number_to);
        if (cek_number.length === 0)
          return reply(`Nomer yang anda masukkan tidak terdaftar di WhatsApp!`);
        number_to = cek_number[0].jid;
        if (sender === number_to)
          return reply(`Jangan kirim ke diri sendiri dong🥲`);
        if (botNumber === number_to)
          return reply(`Jangan kirim ke nomer bot kak🥲`);
        var buttonMessage = {
          text: `Hai kamu menerima pesan Rahasia nih\n\nDari : *${sender_name}*\nIsi Pesannya :\n${msg_send}`,
          buttons: [
            {
              buttonId: `${prefix}balas_secreto ${msg_send}`,
        ...
        reply(
          `Sukses mengirim pesan Rahasia ke nomer tersebut, silahkan tunggu jawaban dari penerima pesan, jika sudah dibalas bot akan otomatis mengirim balasannya!`
        );
        break;
      ...
      case prefix + "connect_chat":
        ...
        if (cek_chat !== undefined) {
          if (cek_chat.status === "WAIT") {
            if (cek_chat.a === sender)
              return reply(
                `Kamu sedang menunggu konfirmasi dari nomer yang tadi, ketik ${prefix}stopchat untuk membatalkan chat sebelumnya`
              );
            if (cek_chat.b === sender)
              return reply(
                `Sebelumnya ada yang mengajak kamu untuk chatan, silahkan cek pesan diatas lalu pencet tombol Terima atau Tolak\nJika sudah tidak ada pesannya, silahkan ketik ${prefix}batalchat`
              );
          }
          if (cek_chat.status === "CHAT")
            return reply(
              `Kamu sedang berada dalam sesi chat, ketik ${prefix}stopchat untuk memberhentikan sesi chat sebelumnya`
            );
        }
        ...
        reply(
          `Sukses mengirim permintaan chat dengan nomer tersebut, tunggu konfirmasi dari dia...`
        );
        break;
      case prefix + "acc_chat":
        ...
        if (quotedMsg.type !== "buttonsMessage") return;
        if (msg.type !== "buttonsResponseMessage") return;
        if (isChat(sender, chat_with) === undefined)
          return reply(`Kamu sedang tidak berada didalam sesi chat manapun!`);
        var data1 = isChat(sender, chat_with);
        if (data1.id !== args[1])
          return reply(`Tombol yang kamu pencet sudah tidak berlaku lagi`);
        ...
        reply(
          `Sukses menerima permintaan chat, silahkan kirim pesan\n\nketik ${prefix}stopchat untuk memberhentikan sesi obrolan!`
        );
        sendMess(
          data1.a,
          `Penerima telah menyetujui permintaan kamu, silahkan kirim pesan\n\nketik ${prefix}stopchat untuk memberhentikan sesi obrolan!`
        );
        break;
      case prefix + "dn_acc":
        if (!isQuotedMsg) return;
        ...
        if (data2.id !== args[1])
          return reply(`Tombol yang kamu pencet sudah tidak berlaku lagi`);
        if (data2.status === "CHAT")
          return reply(
            `Tombol yang kamu pencet sudah tidak berlaku lagi, silahkan ketik ${prefix}stopchat untuk menghentikan sesi chat ini`
          );
      ...
      case prefix + "stopchat":
        ...
        if (cek_data.status === "WAIT") {
          ...
          if (cek_data.a === sender) {
            sendMess(
              cek_data.b,
              `Seseorang telah membatalkan permintaan chatnya denganmu`
            );
            ...
            sendMess(
              cek_data.a,
              `Seseorang telah menolak permintaan chat kamu`
            );
            chat_with.splice(posi_x, 1);
            fs.writeFileSync(
              "./database/chatwith.json",
          ...
          if (cek_data.a === sender) {
            sendMess(
              cek_data.b,
              `Yahh, lawan chat kamu sudah memberhentikan sesi chat ini🥲`
            );
